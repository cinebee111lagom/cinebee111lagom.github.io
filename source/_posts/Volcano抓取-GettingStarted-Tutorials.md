...
**来源**：<https://volcano.sh/zh-Hans/docs/GettingStarted/Tutorials>
...
您需要一个已成功安装Volcano组件的Kubernetes集群。如果尚未安装Volcano，请参考
安装文档
。

## 快速入门：部署一个Volcano Job

本快速入门指南将引导您部署一个简单的Volcano Job。如果未指定队列，Volcano Job默认使用
default
队列。
...
此Job会创建三个Pod，并将它们作为一个组进行调度。Pod模板使用了一个简单的
busybox
容器，并休眠100秒。当Pod完成后，Job的状态也会转为完成。
...
一旦Pod完成，VolcanoJob中的
TaskCompleted
策略将触发
CompleteJob
操作，这会将VolcanoJob的阶段转换为
Completed
：
...
:
volcano
# 关键：确保此Deployment的Pod使用Volcano调度器
containers
:
-
name
...
"-c"
,
"echo 'Hello Volcano from Deployment'; sleep 3600"
]
# 一个长时间运行的命令，用于演示
resources
:
...
当您应用带有
scheduling.volcano.sh/group-min-member
注解的Deployment（或StatefulSet）时，Volcano会自动创建一个PodGroup资源。此PodGroup负责为属于该工作负载的Pod强制执行成组调度约束。
...
lastTransitionTime
:
"2025-05-28T09:08:13Z"
reason
:
tasks in gang are ready to be scheduled
status
:
"True"
transitionID
...
您将观察到Volcano调度器会确保至少
minMember
（本例中为2）个Pod能够一起调度，然后才允许此Deployment中的任何Pod启动。如果资源不足以满足这些Pod的需求，它们将保持
Pending
状态。
...
让我们创建一个名为
development-queue
的队列，并为其指定特定的CPU能力（capability）。分配给此队列的作业将竞争该队列定义的能力范围内的资源。
...
> 完整与最新内容以官方文档为准：[快速开始](https://volcano.sh/zh-Hans/docs/GettingStarted/Tutorials)
