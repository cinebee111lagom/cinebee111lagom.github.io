...
**来源**：<https://volcano.sh/zh-Hans/docs/Concepts/podgroup>
...
minMember表示该podgroup下
最少
需要运行的pod或任务数量。如果集群资源不满足miniMember数量任务的运行需求，调度器将不会调度任何一个该podgroup
内的任务。
...
priorityClassName表示该podgroup的优先级，用于调度器为该queue中所有podgroup进行调度时进行排序。
...
unknown表示该podgroup中
minMember
数量的pod或任务分为2种状态，部分处于running状态，部分没有被调度。没有被调度的原因可能是资源不够等。调度
...
> 完整与最新内容以官方文档为准：[PodGroup](https://volcano.sh/zh-Hans/docs/Concepts/podgroup)
