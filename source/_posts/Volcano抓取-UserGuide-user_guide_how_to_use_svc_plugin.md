---
title: Volcano 抓取：Volcano Job 插件 -- SVC 用户指南
...
**来源**：<https://volcano.sh/zh-Hans/docs/UserGuide/user_guide_how_to_use_svc_plugin>
...
SVC 插件
用于实现 Volcano Job 内 Pod 之间的通信，这在
TensorFlow
、
MPI
等分布式工作负载中是必需的。例如，TensorFlow 作业中的
ps
与
worker
需要相互访问。启用
svc
插件后，同一 Job 内的 Pod 可通过域名互相访问。
...
配置
svc
插件后，会自动为 Job 下所有容器（含 initContainers）注入环境变量
VC_%s_NUM
。其中
%s
为 Pod 所属
...
，变量值为该 Task 的
副本数
。环境变量数量取决于 Task 数量；多数 AI/大数据作业包含两个角色，通常为
2
（例如 Spark 的
driver
与
...
为 Pod 所属
Task 名称
，变量值为该 Task 下所有 Pod 的域名列表。环境变量数量同样取决于 Task 数量；例如 TensorFlow 作业通常包含
ps
与
worker
两个 Task。
...
会自动创建名为
{job-name}-svc
的 ConfigMap，记录各 Task 副本数及 Pod 域名，并挂载到 Job 下所有 Pod，在
/etc/volcano/
目录下提供主机列表文件。
...
:
VC_PS_HOSTS
## 环境变量 VC_PS_HOSTS 包含所有 ps Pod 的域名
valueFrom
:
configMapKeyRef
:
...
:
VC_WORKER_HOSTS
## 环境变量 VC_WORKER_HOSTS 包含所有 worker Pod 的域名
valueFrom
:
configMapKeyRef
:
...
:
VC_WORKER_NUM
## 环境变量 VC_WORKER_NUM 为 worker Task 的副本数
valueFrom
:
configMapKeyRef
:
...
volumeMounts
:
## 将 Job 对应的 ConfigMap 挂载到 /etc/volcano，其中包含各 Task 的主机列表文件
-
mountPath
:
/etc/volcano
...
为该 Job 自动创建的 ConfigMap
tensorflow-dist-mnist-svc
如下：
...
> 完整与最新内容以官方文档为准：[Volcano Job 插件 -- SVC 用户指南](https://volcano.sh/zh-Hans/docs/UserGuide/user_guide_how_to_use_svc_plugin)
