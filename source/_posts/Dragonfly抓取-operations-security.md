---
title: Dragonfly 抓取：Security
date: 2026-09-14 09:35:00
tags:
  - Dragonfly
  - 抓取
  - 文档
...
**来源**：<https://d7y.io/docs/next/operations/security/>
...
Cert-manager will generate a self-signed CA certificate and a server certificate signed by the CA certificate.

Cert-manager will automatically renew the CA certificate and server certificate every 2/3 of their validity period.

Dfdaemon/manager/scheduler will mount the CA certificate and server certificate from the K8s Secret.

Before dfdaemon connects to manager or scheduler, the two side will initiate a TLS handshake with mutual validation.

After TLS handshake, the two side will establish encrypted channel.
...
If you need to use
HTTPS
for the manager console, you must configure the certificate.
Configure
manager.yaml
, the default path is
...
Notice: It is recommended to use
HTTPS
.
...
You can configure the JWT for signing in the manager console. Configure
manager.yaml
,
the default path is
/etc/dragonfly/manager.yaml
...
with a self-signed certificate. Configure
manager.yaml
,
the default path is
/etc/dragonfly/manager.yaml
...
If you need to use
HTTPS
proxy, you must configure the certificate,
...
the state of the system and the impact of changes, refer to
Monitoring
.
...
> 完整与最新内容以官方文档为准：[Security](https://d7y.io/docs/next/operations/security/)
