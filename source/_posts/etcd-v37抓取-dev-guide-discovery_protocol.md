...
本文由批量爬取 [etcd v3.7 文档](https://etcd.io/docs/v3.7/dev-guide/discovery_protocol/) 自动生成，保留原文结构要点，便于检索与对照。

**来源**：<https://etcd.io/docs/v3.7/dev-guide/discovery_protocol/>
...
. If
http://example.com
hosts an etcd cluster for discovery service, a full URL to discovery keyspace will be
http://example.com/v2/keys/_etcd/registry
...
Given the discovery URL, use it as
-discovery
flag and bring up etcd processes. Every etcd process will follow this next few steps internally if given a
-discovery
flag.
...
The wait process is described in detail in the
...
CoreOS Inc. hosts a public discovery service at
https://discovery.etcd.io/
, which provides some nice features for ease of use.

### Mask key prefix

Public discovery service will redirect
https://discovery.etcd.io/${UUID}
to etcd cluster behind for the key at
/v2/keys/_etcd/registry
...
The generation process in the service follows the steps from
Creating a New Discovery Token
to
Specifying the Expected Cluster Size
.
...
. It could be used to build a custom discovery service.
...
> 完整与最新内容以官方文档为准：[Discovery service protocol](https://etcd.io/docs/v3.7/dev-guide/discovery_protocol/)
