---
title: Dragonfly 抓取：Dfget
date: 2026-09-14 09:58:00
tags:
  - Dragonfly
  - 抓取
  - 文档
...
**来源**：<https://d7y.io/docs/next/reference/commands/client/dfget/>

---

dfget
is the command-line tool of Dragonfly used to download and upload files.
...
Install Dfget in the Container Image, refer to
...
Once set up, use the dfget command with the
--transfer-from-dfdaemon
flag to download files:
...
--transfer-from-dfdaemon
specify whether to transfer the content of downloading file from dfdaemon's unix domain
socket. If it is
true
, dfget will call dfdaemon to download the file, and dfdaemon will return the content
of downloading file to dfget via unix domain socket, and dfget will copy the content to the output path.
If it is
false
, dfdaemon will download the file and hardlink or copy the file to the output path.

### Download with different protocols
...
Note: The endpoint does not require
BucketName-APPID
, just --storage-endpoint=cos.region.myqcloud.com.
...
> 完整与最新内容以官方文档为准：[Dfget](https://d7y.io/docs/next/reference/commands/client/dfget/)
